...
void dump_schema_data(MYSQL *conn, char *database, char *table, char *filename) {
	...
	/* There should never be more than one row */
	row = mysql_fetch_row(result);
        /* xuzhifa Modify the start      */
        int prefixLen=strlen(table)+14;


        /* xuzhifa Modify the end    */
	g_string_append(statement, row[1]);

	g_string_append(statement, ";\n");
        g_string_insert(statement, prefixLen, "_Bu");
...
void dump_view_data(MYSQL *conn, char *database, char *table, char *filename, char *filename2) {
	...
	g_string_set_size(statement, 0);
	g_string_append_printf(statement, "CREATE TABLE `%s_Bu`(\n", table);
	row = mysql_fetch_row(result);
...
guint64 dump_table_data(MYSQL * conn, FILE *file, char *database, char *table, char *where, char *filename)
{
	...
	while ((row = mysql_fetch_row(result))) {
		...
		if (!statement->len){
			...
			if (complete_insert) {
				g_string_printf(statement, "INSERT INTO `%s_Bu` (", table);
				...
				g_string_printf(statement, "INSERT INTO `%s_Bu` VALUES", table);
			}
			num_rows_st = 0;
		}
